...
<section id="bo_w">
    ...
    <div class="tbl_frm01 tbl_wrap">
		...
		<?php $i = 0;?>
        <tr>
            <th scope="row">Before 이미지 <span style="color:red">*</span><strong class="sound_only">필수</strong></th>
            <td>
                ...
                <input type="file" name="bf_file[]" title="파일첨부 <?php echo $i+1 ?> :  용량 <?php echo $upload_max_filesize ?> 이하만 업로드 가능" required class="frm_file frm_input required">
                <?php if ($is_file_content) { ?>
                <input type="hidden" name="bf_content[]" value="before" placeholder="파일 설명을 입력해주세요." class="frm_file frm_input" size="50">
                <?php } ?>
        ...
        <tr>
            <th scope="row">After 이미지 <span style="color:red">*</span><strong class="sound_only">필수</strong></th>
            <td>
                <p class="fz_14 ls_2 lh_16 normal m_5" style="color:#ff0000;">★ 이미지는 가로 : 300px , 세로 : 400px 로 등록해주시기 바랍니다.</p>
                <p class="fz_14 ls_2 lh_16 normal m_10" style="width:60%; color:#ff0000;">이미지 사이즈 같은 경우 비율에 맞게 자동으로 사이즈가 변환되기 때문에 가로, 세로 사이즈가 위 사이즈보다 크거나 비율이 다를 경우 이미지에 짤리는 부분이 생기거나 이미지가 깨져보일 수 있습니다.</p>
                <input type="file" name="bf_file[]" title="파일첨부 <?php echo $i+1 ?> :  용량 <?php echo $upload_max_filesize ?> 이하만 업로드 가능" required class="frm_file frm_input required">
