...
<style>
    ...
    .btn_bo_adm input{border-radius: 5px; color: white; background: #ad2128; padding:0 10px; width: 100px; height: 33px; border:none; line-height:30px}
</style>
<!-- 게시판 목록 시작 { -->
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <?php if ($is_category) { 
		//a href 값 오류 보정
		$category_option = str_replace('G5_ADMIN_BBS_URL/', '', $category_option);
		?>
    <nav id="bo_cate">
        <ul id="bo_cate_ul">
    ...
    <div class="bbs_head01 bbs_wrap">
        ...
        for ($i=0; $i<count($list); $i++) {
         ?>
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>" <?php if($list[$i]['wr_7'] == '확인중'){ echo "style='background:#fefcf3'"; }?>>
            <?php if ($is_checkbox) { ?>
            <td class="td_chk">
			...
			<td class="td_setting">
				<a href="<?php echo G5_URL ?>/admin/bbs/write.php?w=u&bo_table=<?=$bo_table?>&wr_id=<?php echo $list[$i]['wr_id'] ?>&page=<?=$page?>&sst=<?=$sst?>&sfl=<?=$sfl?>&stx=<?=$stx?>"><img src="<?php echo G5_URL ?>/img/db_modify.png" alt="수정"></a> / <img src="<?php echo G5_URL ?>/img/db_delete.png" alt="삭제" onclick="wr_id_delete('<?php echo $list[$i]['wr_id']; ?>')" style="cursor:pointer"/>
			</td>
        </tr>
        <?php } ?>
