	...
	<div class="tbl_head01 type02">
		<table>
			...
			<tbody>
					...
					<td style="padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '0'; ?>px">
						...
						<a href="<?=G5_URL ?>/admin/bbs/write.php?w=u&bo_table=<?=$bo_table?>&wr_id=<?=$list[$i][wr_id] ?>">
							...
							<?php
							 ...
							 ?>
							/ <?php if($list[$i]['wr_5'] == '구월점' && $list[$i]['wr_6'] == ''){ ?>
				...
				<?php } ?>
				<?php if (count($list) == 0) { echo '<tr><td colspan="'.$colspan.'" class="empty_table" style="margin-left:auto; margin-right:auto; text-align:center;">등록된 게시물이 없습니다.</td></tr>'; } ?>
			</tbody>
		</table>
	</div>
