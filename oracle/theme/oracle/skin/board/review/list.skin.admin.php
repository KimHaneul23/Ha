...
<style>
    ...
    .review_ba_img_wrap{position:relative; width:100%; max-width: 400px; display:flex; justify-content:center; align-items:flex-start;}
			...
			<tbody>
				...
				<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> <?php echo $lt_class ?>">
					...
					<td style="font-size:16px; padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '0'; ?>px">
						<a href="<?=G5_URL ?>/admin/bbs/write.php?w=u&bo_table=<?=$bo_table?>&wr_id=<?=$list[$i][wr_id] ?>">
							<span style="color:#0f594e; font-weight:600;">[<?php echo $list[$i]['ca_name'] ?>]</span> <?php echo $list[$i]['subject'] ?>
						</a>
					</td>
