...
$(document).ready(function(){
	...
	chart('chart18', g5_url + "/admin/json/statis_chart1.php", 3, color12, title, dateformat3, direction, 1);
	chart('chart19', g5_url + "/admin/json/statis_chart2.php", 6, color10, title, dateformat, direction, 1);
	chart('chart20', g5_url + "/admin/json/statis_chart3.php", 6, color2, title2, dateformat2, direction, 1);
...
function data_excel(type, device, st_date, ed_date,sta_type, page){
	...
	switch(type){
		case 2: // 상담신청 주간
			url2 = g5_url + '/admin/json/statis_chart5.php?device='+device+'&st_date='+st_date+'&ed_date='+ed_date+'&sta_type='+sta_type;
			break;
		case 3: // 상담신청 월간
			url3 = g5_url + '/admin/json/statis_chart6.php?device='+device+'&st_date='+st_date+'&ed_date='+ed_date+'&sta_type='+sta_type;
			break;
		case 4: // 상담신청 요일별
			url4 = g5_url + '/admin/json/statis_chart7.php?device='+device+'&st_date='+st_date+'&ed_date='+ed_date+'&sta_type='+sta_type;
			break;
		default: // 상담신청 일간
			url1 = g5_url + '/admin/json/statis_day.php';
			break;
	}
	...
	if(type = 1){
		$.ajax({
			...
			success : function(data){
				location.href=g5_url +'/admin/json/statis.excel1.php?data='+JSON.stringify(data)+"&device="+device+"&st_date="+st_date+"&ed_date="+ed_date;
			}
		});
	}
	...
	if(type = 2){
		$.ajax({
			...
			type: 'POST',
			success : function(data){
				location.href=g5_url +'/admin/json/statis.excel2.php?data='+JSON.stringify(data)+"&device="+device+"&st_date="+st_date+"&ed_date="+ed_date+"&sta_type="+sta_type;
			}
		});
	}
	...
	if(type = 3){
		...
		$.ajax({
			...
			type: 'POST',
			success : function(data){
				location.href=g5_url +'/admin/json/statis.excel3.php?data='+JSON.stringify(data)+"&device="+device+"&st_date="+st_date+"&ed_date="+ed_date+"&sta_type="+sta_type;
			}
		});
	}
	...
	if(type = 4){
		...
		$.ajax({
			...
			success : function(data){
				//console.log(data);
				location.href=g5_url +'/admin/json/statis.excel4.php?data='+JSON.stringify(data)+"&device="+device+"&st_date="+st_date+"&ed_date="+ed_date+"&sta_type="+sta_type;
			}
		});
	}
}
...
function data_statis(type, device, st_date, ed_date,sta_type, page){
	...
	switch(type){
		case 2: // 상담신청 주간
			...
			chart('chart21', g5_url + '/admin/json/statis_chart5.php?device='+device+'&st_date='+st_date+'&ed_date='+ed_date+'&sta_type='+sta_type, 1, color_2, title, '', direction, 1, 'week_title','week');
			break;
		case 3: // 상담신청 월간
			if(st_date == ''){
				 var today = new Date();
			...
			if(!st_date) st_date = $("#area_4 form input[name='start_date']").val()
			if(!ed_date) ed_date = $("#area_4 form input[name='end_date']").val()
			chart('chart22', g5_url + '/admin/json/statis_chart6.php?device='+device+'&st_date='+st_date+'&ed_date='+ed_date+'&sta_type='+sta_type, 1, color_3, title, '', direction, 1, 'month_title','month');
			break;
		case 4: // 상담신청 요일별
			...
			if(!st_date) st_date = $("#area_4 form input[name='start_date']").val()
			if(!ed_date) ed_date = $("#area_4 form input[name='end_date']").val()
			chart('chart23', g5_url + '/admin/json/statis_chart7.php?device='+device+'&st_date='+st_date+'&ed_date='+ed_date+'&sta_type='+sta_type, 1, color_4, title, '', direction, 1, 'dayofweek_title','weekday');
			break;
		default: // 상담신청 일간
			if(ed_date == '') {
				ed_date = moment().subtract(1,'day').format('YYYY-MM-DD'); 
			...
			$.ajax({
				url : g5_url + '/admin/json/statis_day.php',
				dataType: 'json',
				async : false,
				...
				success : function(data){
					...
					$.ajax({
						url : g5_url + '/admin/json/statis_paging.php',
						async : false,
						data:{device : device, st_date : st_date, ed_date : ed_date, total : total, page : page, sta_type : sta_type},
						type: 'POST',
						success : function(data){
...
function chart21(type, device, st_date, ed_date,sta_type, page){
	...
	$.ajax({
		url : g5_url + '/admin/json/statis_chart5.php',
		dataType: 'json',
		data:{device : device, st_date : st_date, ed_date : ed_date,sta_type : sta_type},
		type: 'POST',
		success : function(data){
...
function all_consul(yesterday){
	$.ajax({
		url : g5_url + '/admin/json/all_consul.php',
		dataType: 'json',
...
function mng(yesterday){
	$.ajax({
		url : g5_url + '/admin/json/mng_count.php',
		dataType: 'json',
		data:{yesterday:yesterday},
		type: 'POST',
		success : function(data){
