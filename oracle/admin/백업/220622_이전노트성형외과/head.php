			...
			<div class="left">
				<div class="left_menu">
					...
					<div class="menu_list">
						<ul>
						<?php
							$sk_text = array(/*'통계 및 카운터',*/'상담관리','회원관리','콘텐츠 관리','게시판 관리','설정관리');
							...
							for($i=0; $i<count($sk_text); $i++) {
								$active = '';
								/*if($sk_text[$i] == '통계 및 카운터') {
									$sk_url = array('visit.php','page.php');
									$sk_text2 = array('방문자 접속 통계','페이지별 접속 통계');
								}*/
								...
								if($sk_text[$i] == '콘텐츠 관리') {
									$sk_url = array('bbs/board.php?bo_table=doctor','bbs/board.php?bo_table=equipment','bbs/board.php?bo_table=event');
									$sk_text2 = array('피부과 전문의','장비 소개','이벤트');
								}
								...
								if($sk_text[$i] == '설정관리') {
									$sk_url = array('newwinlist.php','config_form.php');
									$sk_text2 = array('팝업레이어관리','기본설정');
								}
						...
						<?php } ?>
							...
							<li>
								...
								<ul class="hidden_menu">
									<li><a href="<?php echo G5_URL; ?>/admin/bbs/board.php?bo_table=online_consulting" target="_self">온라인상담</a></li>
