...
for($i=0;$row=sql_fetch_array($qry);$i++){
	...
	else if(strpos($row['vi_referer'], 'www.google.co') !== false) $json['result2'][$i]['referer'] = '구글 / 검색';
...
for($i=0;$row=sql_fetch_array($qry);$i++){
	...
	else if(strpos($row['vi_referer'], 'www.google.co') !== false) $json['result3'][$i]['referer'] = '구글 / 검색';
	...
	$json['result3'][$i]['referer'] .= "-".substr(urldecode($s_tmp[0]), 0, 30);
