...
$(document).ready(function(){
	...
	// 방문자 통계 및 카운터
	chart('chart6', g5_url + "/admin/json/index_chart1.php", 6, color, title1, dateformat, direction, 1);
	chart('chart7', g5_url + "/admin/json/index_chart2.php", 6, color, title2, dateformat, direction, 1);
				...
				success : function(data){
					...
					for(var i=0; i<data.length; i++){
						...
						if(data[i].per > 0) {
							...
							html += '</div><div style="float:left; width: 100%;"><span style="font-size:10px;">(3개월전 '+data[i].prev_date+' / '+number_format(data[i].prev_cnt)+')</span></div></td>';
