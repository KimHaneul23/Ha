...
$(document).ready(function(){
	...
	total_top();
	total_bottom('', '', yesterday, yesterday);
...
function total_top(f){
	...
	var st_date = '';
	if($('input[name="start_date"]', f).val() != yesterday) st_date = $('input[name="start_date"]', f).val();
	var ed_date = '';
	if($('input[name="end_date"]', f).val() != yesterday) ed_date = $('input[name="end_date"]', f).val();
	var order = $('select[name="order"]').val();

	var url1 =  g5_url + '/admin/json/page_chart1.php?device='+device+'&st_date='+st_date+'&ed_date='+ed_date+'&order='+order;
...
setTimeout(function() {
	$.ajax({
		url : g5_url + '/admin/json/page_board.php',
		type : 'post',
		dataType : 'json',
...
function page_excel(f){
	...
	$.ajax({
		...
		success : function(data){
			//console.log(data);
			location.href=g5_url +'/admin/json/page.excel1.php?data='+JSON.stringify(data)+"&st_date="+st_date+"&ed_date="+ed_date+"&device="+device;
		}
	});
}
...
function page_excel2(f){
	...
	console.log(type);
	$.ajax({
		url : g5_url + '/admin/json/page_requri.php',
		dataType : 'json',
		async : false,
		type : 'post',
		data : {device : device, st_date : st_date, ed_date : ed_date, order : order, type : type, page : page},
		success : function(data){
			location.href=g5_url +'/admin/json/page.excel2.php?&st_date='+st_date+"&ed_date="+ed_date+"&device="+device+"&order="+order+"&type="+type+"&catagory="+catagory;
		}
	});
	...
	$.ajax({
		...
		success : function(data){
			...
			if(total2 > 0){
				for(var i=0; i<data2.length; i++){
					html2 += '<tr><td>'+(i+1)+'</td>';
					html2 += '<td data-tooltip-text='+data2[i].referer+'>'+data2[i].referer+'</td>';
			...
			$.ajax({
				url : g5_url + '/admin/json/page_paging.php',
				type : 'post',
