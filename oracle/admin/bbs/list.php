...
if ($board['bo_use_category']) {
    ...
    if ($bo_table == "realselfie" || $bo_table == "bna") {
        ...
        for ($i=0; $i<count($categories); $i++) {
            $category = trim($categories[$i]);
            if ($category=='') continue;
            //$category_option .= '<li><a href="'.($category_href."&sfl=ca_name%2C1&&stx=".urlencode($category)).'"'; 
            $category_option .= '<li';
            $category_msg = '';
            if ($category==$stx) {
                $category_option .= ' id="bo_cate_on"';
            ...
            $category_option .= '><a href="'.($category_href."&sfl=ca_name%2C1&&stx=".urlencode($category)).'">'.$category_msg.$category.'</a></li>';
        }
...
if ($member['mb_level'] >= $board['bo_write_level']) {
    $write_href = short_url_clean(G5_URL.'/admin/bbs/write.php?bo_table='.$bo_table);
}
