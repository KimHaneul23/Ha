...
<style>
    ...
    .review_ba_img_wrap{position:relative; width:100%; max-width: 400px; display:flex; justify-content:center; align-items:flex-start; gap:0 0.4rem;}
    .review_ba_img{width:50%; max-width:200px;}
    .review_ba_img > img{max-width:100%;}
    
    .local_ov02.local_ov{margin:0 0 1rem;}
</style>


<div class="sub-headline">
	<h1 class="sub-headline-tit"><a href="" style="pointer-events:none;">공지 · 이벤트</a></h1>
</div>
			...
			<tbody>
				...
				<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> <?php echo $lt_class ?>">
						...
						<div class="gall_img" >
							<a href="./write.php?w=u&bo_table=<?=$bo_table?>&wr_id=<?=$list[$i]['wr_id'] ?>">
                                <div class="review_ba_img_wrap">
                                    ...
                                    <div class="review_ba_img review_after_img">
                                        <?php echo $file_src2; ?>
                                    </div>
                                </div>
							</a>
					...
					<td style="font-size:16px; padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '0'; ?>px">
						<a href="./write.php?w=u&bo_table=<?=$bo_table?>&wr_id=<?=$list[$i]['wr_id'] ?>" style="text-decoration:none;">
							<span style="font-weight:600;">[<?php echo $list[$i]['ca_name'] ?>]</span> <?php echo $list[$i]['subject'] ?>
						</a>
					</td>
					<td class="td_datetime"><?php echo $list[$i]['datetime'] ?></td>
				</tr>
				<?php } ?>
				<?php if (count($list) == 0) { echo '<tr><td colspan="'.$colspan.'" class="empty_table" style="margin-left:auto; margin-right:auto; text-align:center;">전후사진 준비 중입니다.</td></tr>'; } ?>
			</tbody>
		</table>
	</div>
