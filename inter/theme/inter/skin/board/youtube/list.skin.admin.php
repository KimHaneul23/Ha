...
<form name="fboardlist"  id="fboardlist" action="<?php echo G5_BBS_URL; ?>/board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
		...
		<table>
			...
			<thead>
				<tr>
					...
					<?php } ?>
					<th scope="col">번호</th>
					<th scope="col">채널 로고</th>
					<th scope="col">썸네일</th>
					<th scope="col">제목/내용</th>
					<th scope="col"><?php echo subject_sort_link('wr_datetime', $qstr2, 1) ?>등록일</th>
				</tr>
			</thead>
			<tbody>
				<?php 
				...
				<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> <?php echo $lt_class ?>">
					...
					<td>
						<div class="gall_img" style="display:flex; justify-content:center; align-items:center; height:80px;">
							...
							<?php } else {
								...
								if($thumb['src']) {
									$img_content = '<img src="'.$thumb['ori'].'" alt="'.$thumb['alt'].'" style="max-width:100%; height:80px;">';
								} else {
					...
					<td>
						<div class="gall_img" style="display:flex; align-items:center; <?php if ($board['bo_gallery_height'] > 0) echo 'height:'.$board['bo_gallery_height'].'px;max-height:'.$board['bo_gallery_height'].'px'; ?>">
							...
							<?php } else {
								...
								if($list[$i]['wr_1']) {
									$img_content = '<img src="https://img.youtube.com/vi/'.$list[$i]['wr_1'].'/mqdefault.jpg" style="width:100%;">';
								} else {
									$img_content = '<span class="no_image" style="'.$line_height_style.'">no image</span>';
								}
					...
					<td class="td_subject">
						<a href="./write.php?w=u&bo_table=<?=$bo_table?>&wr_id=<?=$list[$i][wr_id] ?>">
							...
							<?php
							 ...
							 ?>
							<p class="tit"><?php echo $list[$i]['subject'] ?></p>
							<p class="txt01 ellipsis line3"><?php echo nl2br($list[$i]['wr_content']); ?></p>
						</a>
					</td>
	...
	<div class="btn_fixed_top">
		...
		<?php if ($is_checkbox) { ?>
<!--		<button type="submit" name="btn_submit" value="선택수정" onclick="document.pressed=this.value" class="btn type02">선택수정</button>-->
