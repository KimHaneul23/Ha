...
if ($board['bo_use_category']) {
    ...
    if(!is_adm_page()){
		...
		$category_option .= '<li><a href="'.$category_href.'#href_id"';
	}else {
		$category_href = G5_ADMIN_URL.'/board.php?bo_table='.$bo_table;
	...
	if ($bo_table == "c_article") {
		
        for ($i=0; $i<count($categories); $i++) {
                $category = trim($categories[$i]);
                if ($category=='') continue;
                if(!is_adm_page()){
                    $category_option .= '<li><a href="'.($category_href."&sca=".urlencode($category)).'#href_id"';
                }else {
    ...
    } else if ($bo_table == "before_after") {
		
        for ($i=0; $i<count($categories); $i++) {
            $category = trim($categories[$i]);
			if ($category=='') continue;
			if(!is_adm_page()){
				$category_option .= '<li><a href="'.($category_href."&sca=".urlencode($category)).'&sop=or#href_id"';
			}else {
				$category_option .= '<li><a href="'.($category_href."&sca=".urlencode($category)).'&sop=or"';
			}
        ...
        for ($i=0; $i<count($categories); $i++) {
                $category = trim($categories[$i]);
                if ($category=='') continue;
                //$category_option .= '<li><a href="'.(get_pretty_url($bo_table,'','sca='.urlencode($category))).'#href_id"';
...
if ($is_search_bbs) {
    ...
    if (isset($min_spt) && $prev_spt >= $min_spt) {
        ...
        $prev_part_href = './board.php?bo_table='.$bo_table.'&'.$qstr1.'&amp;spt='.$prev_spt.'&amp;page=1#href_id';
    ...
    if ($next_spt < 0) {
        $qstr1 = preg_replace($patterns, '', $qstr);
        $next_part_href = './board.php?bo_table='.$bo_table.'&'.$qstr1.'&amp;spt='.$next_spt.'&amp;page=1#href_id';
